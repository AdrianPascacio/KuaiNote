...
public class Repeater_PopUpWindow implements Every_X_Day_WindowPopUp.OnValueSelectedListener_Every_X_Day, Every_X_Day_WindowPopUp.PopupDismissListener_Repeater_X_Day, Every_X_Hour_WindowPopUp.OnValueSelectedListener_Every_X_Hour, Every_X_Hour_WindowPopUp.PopupDismissListener_Repeater_X_Hour,Every_Day_Of_The_Week.OnValueSelectedListener_Every_Day_Of_Week, Every_Day_Of_The_Week.PopupDismissListener_Repeater_Day_Of_Week, Every_Day_Of_Month.OnValueSelectedListener_Every_Day_Of_Month, Every_Day_Of_Month.PopupDismiissListener_Repeater_Every_Day_Of_Month {
    ...
    private int journal_element_reminder_type = 0;
    private int journal_element_reminder_interval = 0;
    ...
    TextView label_none, label_daily, label_every_x_hour, label_every_x_day, label_every_day_of_week, label_every_day_of_month;
    View layout_dim;
    ...
    @Override
    public void OnValueSelected_Repeater_every_x_day(int days_interval) {
        if (listener_repeater_every_x_day != null) {
            listener_repeater_every_x_day.OnValueSelected_Repeater_type_Every_X_Day(5, days_interval); // Devolver el nuevo tipo de repeater
        }

        popupWindow.dismiss();
        if (listener_dismiss_repeater != null) {
            listener_dismiss_repeater.onPopupClosed_Repeater(5); // Devolver el mismo valor`
        }
    }
    ...
    @Override
    public void onPopupClosed_Repeater_X_Day(int salida) {
        layout_dim.setVisibility(View.VISIBLE);
        layout_dim.startAnimation(AnimationLayoutDimDisappear_Normal);
    }
    ...
    public interface OnValueSelectedListener_Repeater_Every_X_Day{
        void OnValueSelected_Repeater_type_Every_X_Day(int reminder_repeater_type, int reminder_repeater_interval);
    }
    ...
    private OnValueSelectedListener_Repeater_Every_X_Day listener_repeater_every_x_day;
    ...
    public void setListener_repeater_every_x_day(OnValueSelectedListener_Repeater_Every_X_Day listener_repeater_every_x_day){
        this.listener_repeater_every_x_day = listener_repeater_every_x_day;
    }
    ...
    public void show(View view_brought, Note note, int n_reminder_type, int n_reminder_interval, int day_picked){
        ...
        journal_element_reminder_interval = n_reminder_interval;
        journal_element_reminder_type = n_reminder_type;
        ...
        label_daily = container.findViewById(R.id.Repeater_Label_Every_Day);
        label_every_x_hour = container.findViewById(R.id.Repeater_Label_Every_X_Hour);
        label_every_x_day = container.findViewById(R.id.Repeater_Label_Every_X_Day);
        ...
        label_daily.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        label_every_x_hour.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Set_Every_X_Hour_Repeater(journal_element_reminder_type, journal_element_reminder_interval);
            }
        });
        label_every_x_day.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                layout_dim.setVisibility(View.VISIBLE);
                layout_dim.startAnimation(AnimationLayoutDimAppear);
                Set_Every_X_Day_Repeater(journal_element_reminder_type, journal_element_reminder_interval);
            }
        });
        label_every_day_of_week.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                layout_dim.setVisibility(View.VISIBLE);
                layout_dim.startAnimation(AnimationLayoutDimAppear);
                Set_Every_Day_Of_The_Week(journal_element_reminder_type, journal_element_reminder_interval);
            }
        });
        label_every_day_of_month.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                layout_dim.setVisibility(View.VISIBLE);
                layout_dim.startAnimation(AnimationLayoutDimAppear);
                Set_Every_Day_Of_The_Month(journal_element_reminder_type, journal_element_reminder_interval, day_picked);
            }
        });
    }
    public void show(View view_brought, Task_Main task, int n_reminder_type, int n_reminder_interval, int day_picked){
        //!! el parametro Note note que es traido de el reminderpopupwindow, no se esta actualizando como espero, se intenta actualizar con un listener, pero no surte efecto, es por esta razon que se incluyeron los parametros de tipo int (n_reminder_type y n_reminder_interval)
        //!! esta funcionando como note y no como el this.note traido desde Reminder_PopUpWindow
        this.view_brought = view_brought;
        ...
        journal_element_reminder_interval = n_reminder_interval;
        journal_element_reminder_type = n_reminder_type;
        original_reminder_type = task.reminder_type;
        original_reminder_interval = task.reminder_interval;
        ...
        if(task.reminder_interval == n_reminder_interval && task.reminder_type == n_reminder_type){
            Highlight_Type_Of_Reminder();
        }else{
            Highlight_Type_Of_Reminder_Not_Updated();
        }
        ...
        if( task.reminder > 0){
            calendar_prev.setTimeInMillis( task.reminder);
            repeat_alarm = task.reminder_type > 0;
        }
        if(repeat_alarm == true){
            //fl_set_repeat_alarm.setBackgroundTintList(ColorStateList.valueOf(ContextCompat.getColor(context, R.color.repeat_alarm_on)));
            //fl_set_repeat_alarm.setBackgroundResource(R.drawable.repeat_normal_2);
        ...
        public void onDismiss(){
            if(!action_took){
                task.reminder_type = original_reminder_type;
                task.reminder_interval = original_reminder_interval;
                Repeater_PopUpWindow.this.listener_dismiss_repeater.onPopupClosed_Repeater(original_reminder_type);
            }
        }
        });
        ...
        label_none.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                popupWindow.dismiss();
                if (listener_dismiss_repeater != null) {
                    listener_dismiss_repeater.onPopupClosed_Repeater(task.reminder_type); // Devolver el mismo valor`
                }
            }
        });
        btn_cancel_reminder_alarm.setOnClickListener(new View.OnClickListener() {
    ...
    private void Highlight_Type_Of_Reminder() {
        label_none.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 0 ? R.color.ex_orange : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_daily.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 1 ? R.color.ex_orange : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_every_x_hour.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 2 ? R.color.ex_orange : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        //!!--cambiar numero:
        label_every_x_day.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 5 ? R.color.ex_orange : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_every_day_of_week.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 3 ? R.color.ex_orange : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_every_day_of_month.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 4 ? R.color.ex_orange : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
    }

    private void Highlight_Type_Of_Reminder_Not_Updated() {
        label_none.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 0 ? R.color.matcha_1_trans : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_daily.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 1 ? R.color.matcha_1_trans : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_every_x_hour.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 2 ? R.color.matcha_1_trans : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        //!!--cambiar numero:
        label_every_x_day.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 5 ? R.color.matcha_1_trans : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_every_day_of_week.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 3 ? R.color.matcha_1_trans : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
        label_every_day_of_month.setTextColor(ContextCompat.getColor(context, journal_element_reminder_type == 4 ? R.color.matcha_1_trans : R.color.item_visualizer_preview_text_color)); ///Ternary Operator and XOR Operator
    }
    ///label_none = 0
    ///label_daily. = 1
    ///label_every_x_hour. = 2
    ///label_every_day_of_week. = 3
    ///label_every_day_of_month. = 4
    ///label_every_x_hour. = 5
    ...
    private void Set_Every_X_Day_Repeater(int note_reminder_type, int note_remider_interval) {
        Every_X_Day_WindowPopUp repeater_PopUp = new Every_X_Day_WindowPopUp(context, position);
        repeater_PopUp.setListener_every_x_day(this);
        repeater_PopUp.setListener_dismiss_every_x_day(this);
