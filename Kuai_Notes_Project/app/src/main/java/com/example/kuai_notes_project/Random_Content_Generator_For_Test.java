...
public class Random_Content_Generator_For_Test {
    ...
    public void Random_Notes_Generator(Context context, int Content_Number){
        ...
        for(int i = Content_Number ; i>=0; i--){
            Randome_Number_Generator result = Generate_Random();
            Fill_Notes(result.random_number, result.end_of_title, result._current_time);
        }
    }

    @NonNull
    private static Randome_Number_Generator Generate_Random() {
        long _current_time = System.currentTimeMillis();
        long _nano_time = System.nanoTime();
        int random_number = (int) (_nano_time & 4095);   /// bitwise & long & 4095 (binary = 111111111111 (1 doce veces)) → para tomar un numero aleatorio entre 0 a 4095
        int end_of_title = random_number & 63;  ///Bitwise & → int & 15 (binary = 1111) → para tomar los numeros menores de 63;
        Randome_Number_Generator result = new Randome_Number_Generator(_current_time, random_number, end_of_title);
        return result;
    }

    private static class Randome_Number_Generator {
        public final long _current_time;
        public final int random_number;
        public final int end_of_title;

        public Randome_Number_Generator(long _current_time, int random_number, int end_of_title) {
            this._current_time = _current_time;
            this.random_number = random_number;
            this.end_of_title = end_of_title;
        }
    }

    private void Fill_Tasks(int i, int random_number, int end_of_title, long _current_time) {
        ...
        String _title = seed_text_midium.substring(random_number, random_number + end_of_title);

        long new_task_id = DB_T.Insert_Task_L_for_test_random_generator(_current_time, _title, _title, random_pin, 0L, 0, 0,random_has_sub_task,random_complete);
        if( random_has_sub_task == true){
            for(int j = 1; j <= 29;j++){
                boolean sub_task_random_complete  = random_complete || ((random_number >> j+2) & 1) == 1; /// OR if ramdom_complete == true; then true; else random
                Log.d("Random", "    Sub Task:  Random complete: " + sub_task_random_complete);
                DB_T.Insert_Task_Sub_L(new_task_id,j+_title,sub_task_random_complete,j);
                if((random_number + i & 1)  == 1) break;
            }
        }
    }

    private void Fill_Notes(int random_number, int end_of_title, long _current_time) {
        int random_start_of_note  = (random_number >> 1) ;
        int random_end_of_note = random_start_of_note & 63;
        int random_pin = random_end_of_note & 1;

        Log.d("Random", "Random end of note: " + random_end_of_note + "    Random pin: " + random_pin);

        String _title = seed_text_midium.substring(random_number, random_number + end_of_title);
        String _note = seed_text_midium.substring(random_start_of_note,random_end_of_note + random_start_of_note);

        DB_N.Insert_Note_L(_current_time, _title, _note, random_pin == 1, 0L, 0, 0);
    }
}
